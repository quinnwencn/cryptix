...
std::optional<PrivateKey> PrivateKey::FromKeyFile(const std::filesystem::path& keyPath) {
    if (std::filesystem::exists(keyPath) == false) {
        CRYPTIX_ERROR(fmt::format("{} not exists.", keyPath.string()));
        return std::nullopt;
    }

    std::ifstream ifs(keyPath);
    if (!ifs.is_open()) {
        CRYPTIX_ERROR(fmt::format("{} open failed.", keyPath.string()));
        return std::nullopt;
    }
...
std::optional<std::vector<uint8_t>> PrivateKey::Sign(const uint8_t* data, size_t size, SignAlgo algo) {
    ...
    if (algo == SignAlgo::RSASSA_PSS) {
        if (::EVP_PKEY_CTX_set_rsa_padding(pkeyCtx, RSA_PKCS1_PSS_PADDING) <= 0) {
            CRYPTIX_ERROR(fmt::format("Set padding failed: {}", ::ERR_error_string(::ERR_get_error(), nullptr)));
            return std::nullopt;
        }
    ...
    if (::EVP_DigestSignUpdate(mdCtx.get(), static_cast<const void*>(data), size) <= 0) {
        CRYPTIX_ERROR(fmt::format("Update sign failed: {}", ::ERR_error_string(::ERR_get_error(), nullptr)));
        return std::nullopt;
    }

    size_t sigLen = 0;
    if (::EVP_DigestSignFinal(mdCtx.get(), nullptr, &sigLen) <= 0) {
        CRYPTIX_ERROR(fmt::format("Finalize sign failed: {}", ::ERR_error_string(::ERR_get_error(), nullptr)));
        return std::nullopt;
    }
